package bit.project.eazip.domain.home;


import java.util.List;
import lombok.Getter;
import lombok.Setter;

@Getter
@Setter
public class HomeDTO {
    private int 	idx;
    private int 	category1;
    private String 	category2;
    private String 	name;
    private String 	dong;
    private Double 	layer;
    private int layer_all;
    private String 	layer_name;
    private String 	direction;
    private String 	directionN;
    private Double 	supply;
    private Double 	exclusive;
    private Double 	supply_pyeong;
    private Double 	exclusice_pyeong;
    private int 	room_number;
    private int 	bathN_number;
    private int 	parking;
    private int 	split_level;
    private String 	move_date;
    private int 	type;
    private int 	price;
    private int 	jeonse;
    private int 	monthly;
    private int 	deposit;
    private int 	loan;
    private int 	loan_type;
    private int 	case_jeonse;
    private int 	case_monthly;
    private int 	management_cost;
    private int 	to_jeonse;
    private String 	ad;
    private String 	feature;
    private String 	agency;
    private int 	use_permission_date;
    private String 	use_year;
    private Double 	lat;
    private Double 	lng;
    private int 	parking2;
    private String 	heating_type;
    private int 	lift;
    private int 	earthquake_resistance;
    private String 	room_type;
    private String 	addr1;
    private String 	addr2;
    private String 	building_name;
    private String image_url;
    private int	aircon_hanging;
    private int	aircon_standing;
    private int	aircon_system;
    private int	bed;
    private int	desk;
    private int	wardrobe;
    private int	closet;
    private int	table;
    private int	couch;
    private int	shoe;
    private int	fridge;
    private int	washing;
    private int	drier;
    private int	shower_booth;
    private int	bath;
    private int	bidet;
    private int	sink;
    private int	dishwasher;
    private int	gas;
    private int	induction;
    private int	microwave;
    private int	oven;
    private int	veranda;
    private int	security;
    private int	videocall;
    private int	interphone;
    private int	cardkey;
    private int	cctv;
    private int	security_private;
    private int	entrance_security;
    private int	window_guard;
    private int	private_bath;
    private int	private_shower;
    private int	fire_alarm;
    private int	delivery_box;
    private int	elevator;
    private int	terrace;
    private int	garden;
    private double sc_bank;
    private double sc_bogun;
    private double sc_bus;
    private double sc_cafe;
    private double sc_cctv;
    private double sc_ddar;
    private double sc_hospital;
    private double sc_laundary;
    private double sc_market;
    private double sc_office;
    private double sc_post;
    private double sc_subway;




<<<<<<< Updated upstream
=======




>>>>>>> Stashed changes
}
